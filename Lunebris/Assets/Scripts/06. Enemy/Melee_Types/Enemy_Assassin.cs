...
public class Enemy_Assassin : Enemy_Base
{
    ...
    [SerializeField] private float dashRange = 100f; // 대쉬 사용 가능 범위 (12 → 15로 증가)
    ...
    [Header("대쉬 물리 설정")]
    [SerializeField] private bool ignoreCollisionDuringDash = true; // 대쉬 중 충돌 무시 여부
    [SerializeField] private LayerMask dashObstacleLayer = -1; // 대쉬 중 충돌 체크할 레이어
    ...
    private Collider[] enemyColliders; // 충돌 무시를 위한 콜라이더들
    ...
    protected override void UpdateBehavior()
    {
        ...
        // 3. 대쉬 범위 밖이면 일반 추적 (UpdateMovement에서 처리)
        // 이 부분은 UpdateMovement()에서 base.UpdateMovement() 호출로 처리됨
    }
    ...
    protected override void UpdateMovement()
    {
        ...
        // 대쉬 범위 밖이고 공격 범위 밖일 때만 일반 추적 이동
        if (distanceToPlayer > dashRange && !IsPlayerInAttackRange())
        {
            // 일반적인 추적 이동 (부모 클래스의 기본 이동 사용)
            base.UpdateMovement();
        }
        else
        {
            // 대쉬 범위나 공격 범위 내에서는 움직임 정지 (대기)
            StopMovement();
        }
    }
    ...
    private void ExecuteDash()
    {
        ...
        Debug.Log($"{enemyName}: 대쉬 실행! 시작: {dashStartPosition}, 목표: {dashTargetPosition}");
    ...
    private void UpdateDash()
    {
        ...
        {
            // 장애물에 막혔을 때 대쉬 종료
            Debug.Log($"{enemyName}: 장애물에 막혀 대쉬 중단");
            EndDash();
        }
    }
    ...
    {
        // 현재 위치에서 목표 위치로의 경로에 장애물이 있는지 체크
    ...
    private void EndDash()
    {
        ...
        Debug.Log($"{enemyName}: 대쉬 완료! 최종 위치: {transform.position}");
